...
|---|---|---|---|---|
| **S1 홈** (`home`) | 대회 탐색·검색·필터 | 검색 인풋, 필터 칩(지역·월·접수가능만), 대회 카드(날짜열·이름·지역·종목 태그·접수상태 칩·**출처/확인일**), 리스트/캘린더 토글 | 읽기 `races` / 쓰기 `SELECT_RACE` | 카드 클릭 → `race` |
...
| 항목 | 값 |
|---|---|
| 대회 수 | **271** (CSV 행수 898은 멀티라인 description 때문 — 파서는 반드시 RFC 4180 호환 사용) |
...
```
크롤 CSV (마라톤온라인+마라톤GO, 271건)
  → backend/normalize_races.py (§4.2 Race 계약으로 출력 정렬)
     · RFC4180 파서 · 종목 표준화 stdEvents · regionCode 파생('충청' 보정)
     · 중복 병합(정규화명+date) · 좌표는 CSV값 사용(100%), 누락 시 geocode.py
  → design/public/data/races.json (Race[])
  → 프론트: sampleData.RAW_RACES 제거, races.json 로드로 교체 (G-01)
```
...
| G-01 | `RAW_RACES` 제거 → 파이프라인 `races.json`(271건) 연결 + regionCode 파생·'충청' 정제 | §6.2 | P0 |
| G-02 | `TODAY='2026-06-18'` 하드코딩 제거 → 실제 현재일 (D-day·접수상태 계산) | design 분석 | P0 |
